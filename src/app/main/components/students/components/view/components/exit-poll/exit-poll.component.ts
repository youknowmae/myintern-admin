...
export class ExitPollComponent {
  ...
  getExitPollDetails() {
    ...
    this.ds
      .get(
        `adviser/students/exit-poll/${user.id}`,
      ...
      .subscribe(
        (response) => {
          ...
          let supervisor =
            exitPollDetails.industry_partner.immediate_supervisor;
          let supervisorFullName = `${supervisor?.first_name || ''} ${
            supervisor?.last_name || ''
          } ${supervisor?.ext_name || ''}`.trim();
          exitPollDetails.industry_partner.immediate_supervisor.full_name =
            supervisorFullName;

          console.log(exitPollDetails);

          exitPollDetails.industry_partner.full_address = `${exitPollDetails.industry_partner.street} ${exitPollDetails.industry_partner.barangay} 
