...
export class ViewComponent {
  ...
  getApplicationDetails() {
    ...
    this.ds.get('adviser/applications/', id).subscribe(
      (applicationDetails) => {
        ...
        applicationDetails.industry_partner.immediate_supervisor.full_name =
          supervisorFullName;
      ...
      (error) => {
        ...
        if (error.status === 404) {
          ...
          this.gs.makeAlert('Not Found!', 'Application not found.', 'error');
        } else {
          this.gs.makeAlert(
            'Oops!',
  ...
  approve() {
    ...
    this.ds
      .post('adviser/applications/accept/', this.applicationDetails.id, null)
      .subscribe(
        (response) => {
          this.isSubmitting = false;
          this.gs.makeAlert('Approved!', response.message);
  ...
  reject() {
    ...
    this.ds
      .post('adviser/applications/reject/', this.applicationDetails.id, null)
      .subscribe(
        (response) => {
          this.isSubmitting = false;
          this.gs.makeAlert('Declined!', response.message);
  ...
  generateEndorsement() {
    this.ds
      .download(
        'adviser/applications/download/endorsement/',
        this.applicationDetails.id
      )
      .subscribe(
        (response: Blob) => {
          saveAs(
            response,
            'GCCCS.ENDORSEMENT.LETTER.' +
              this.applicationDetails.user.first_name +
              '.' +
              this.applicationDetails.user.last_name +
              '.pdf'
          );
